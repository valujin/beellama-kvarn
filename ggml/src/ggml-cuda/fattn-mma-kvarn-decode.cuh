...
static __global__ void ggml_cuda_fattn_kvarn_decode_combine_kernel(
    ...
    // ВОЛНА 42. Барьер обязателен: ниже reduce_sh[tid] перезаписывается частичной
    // суммой знаменателя, а reduce_sh[0] здесь ещё читают все нити блока. Без
    // барьера нить 0, добравшись до записи раньше, чем чужой варп успел прочитать
    // максимум, подменяет m знаменателем. Окно узкое — нить 0 идёт к записи через
    // глобальное чтение partial_meta, а чужие варпы читают сразу после
    // предыдущего барьера, — поэтому на одиннадцати воспроизведениях гонка не
    // проявлялась. Но она настоящая, и цена барьера равна нулю: блок и так
    // синхронизируется двумя строками ниже.
    __syncthreads();
